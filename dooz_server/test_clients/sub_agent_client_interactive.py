...
class InteractiveClient:
    ...
    def __init__(
        self, server_url: str = "ws://localhost:8000", profile: Optional[dict] = None
    ):
    ...
    def print_help(self):
        ...
        print(
            self.color(
                "white", "  send <client_id> <message>  - Send message to client"
            )
        )
        print(
            self.color(
                "white", "  list                       - List all connected clients"
            )
        )
        print(
            self.color(
                "white", "  agents                     - List only agents"
            )
        )
        print(
            self.color("white", "  history                     - Show message history")
        )
        print(
            self.color(
                "white", "  pending                     - Check pending messages"
            )
        )
    ...
    async def connect(self):
        ...
        try:
            ...
            if self.profile:
                print(
                    self.color(
                        "cyan", f"  Profile: {self.profile.get('role', 'unknown')} role"
                    )
                )
            return True
        except Exception as e:
    ...
    async def list_clients(self, role_filter: str = None):
        """List all connected clients via HTTP, optionally filtered by role."""
        import httpx
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if resp.status_code == 200:
                    data = resp.json()
                    role_text = f" ({role_filter})" if role_filter else ""
                    print(
                        self.color("cyan", f"\n📋 Connected Clients{role_text} ({data['total']}):")
                    )
                    for c in data["clients"]:
                        is_me = (
                            self.color("yellow", " (YOU)")
                            if c["client_id"] == self.device_id
                            else ""
                        )
                        ...
                        if profile:
                            ...
                            role_color = "green" if role == "agent" else "blue" if role == "user" else "white"
                            print(f"    {self.color(role_color, f'Role: {role}')}")
                            
                            if skills:
                                print(f"    {self.color('dim', 'Skills:')}")
                                for skill_name, skill_desc in skills:
                                    print(f"      • {self.color('white', skill_name)}: {skill_desc}")
                    print()
                else:
                    print(
                        self.color(
                            "red", f"✗ Failed to list clients: {resp.status_code}"
                        )
                    )
        except Exception as e:
    ...
    def show_history(self):
        ...
        for i, msg in enumerate(self.sent_messages[-10:], 1):
            status = (
                self.color("green", "✓") if msg["success"] else self.color("red", "✗")
            )
        ...
        for i, msg in enumerate(self.received_messages[-10:], 1):
            offline = (
                self.color("magenta", " [OFFLINE]") if msg.get("is_offline") else ""
            )
        ...
        try:
            async with httpx.AsyncClient() as client:
                resp = await client.get(
                    f"http://localhost:8000/messages/pending/{self.device_id}"
                )
                if resp.status_code == 200:
                    data = resp.json()
                    print(
                    ...
                    print(
                        self.color(
                            "red", f"✗ Failed to check pending: {resp.status_code}"
                        )
                    )
        except Exception as e:
    ...
    async def handle_command(self, cmd: str):
        ...
        if command == "send" and len(parts) >= 3:
            ...
            await self.list_clients(role_filter="agent")
...
async def main():
    ...
    client = InteractiveClient(server_url=args.server, profile=profile)
    await client.run()

...
class WebSocketTUI(DoozTUI):
    ...
    def _handle_daemon_message(self, data: dict) -> None:
        ...
        if msg_type == "response":
            ...
            self.get_chat_screen().add_daemon_message(content)
        elif msg_type == "error":
            error_msg = data.get("message", "Unknown error")
            self.get_chat_screen().add_error_message(f"Error: {error_msg}")
        elif msg_type == "pong":
            self.get_chat_screen().add_system_message("Pong received")
        else:
            self.get_chat_screen().add_system_message(str(data))
    
    async def handle_message_submitted(self, event: MessageSubmitted) -> None:
        """Handle message submitted - send to daemon."""
        if self.client and self.client.is_connected:
            ...
            self.get_chat_screen().add_error_message("Not connected to daemon")
